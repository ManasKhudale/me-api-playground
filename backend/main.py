...
from .schemas import ProfileIn, ProfileOut, ProjectOut, ProjectIn
from .auth import require_basic_auth
from .rate_limit import rate_limit

app = FastAPI(title="Me-API Playground", version="1.0.0")

# CORS (allow all by default; configure ALLOWED_ORIGINS env to narrow)
...
def create_profile(data: ProfileIn, db: Session = Depends(get_db)):
    ...
    profile = Profile(
        name=data.name, email=str(data.email), education=data.education,
        github=data.links.get("github"), linkedin=data.links.get("linkedin"),
    ...
    # work
    for w in data.work:
        db.add(Work(profile_id=profile.id, company=w.company, title=w.title,
...
def update_profile(data: ProfileIn, db: Session = Depends(get_db)):
    ...
    # reset projects
    db.query(ProjectLink).filter(ProjectLink.project_id.in_(db.query(Project.id).filter(Project.profile_id==profile.id))).delete(synchronize_session=False)
    db.query(ProjectSkill).filter(ProjectSkill.project_id.in_(db.query(Project.id).filter(Project.profile_id==profile.id))).delete(synchronize_session=False)
...
def list_projects(skill: Optional[str] = None, q: Optional[str] = None,
...
def top_skills(limit: int = 5, db: Session = Depends(get_db)):
    # Top skills by number of associated projects; fallback to profile skills
    stmt = select(Skill.name, func.count(ProjectSkill.project_id).label("count"))\
        .join(ProjectSkill, ProjectSkill.skill_id == Skill.id, isouter=True)\
        .group_by(Skill.id).order_by(func.count(ProjectSkill.project_id).desc(), Skill.name).limit(limit)
...
def search(q: str, limit: int = 20, offset: int = 0, db: Session = Depends(get_db)):
    like = f"%{q}%"
    # Search across projects title/description and skills
    proj_stmt = select(Project).where((Project.title.ilike(like)) | (Project.description.ilike(like)))\
        .limit(limit).offset(offset)
    projects = db.execute(proj_stmt).scalars().all()
    skills = db.execute(select(Skill).where(Skill.name.ilike(like)).limit(limit).offset(offset)).scalars().all()
    return {
...
# --- Static frontend (optional). Serve ../frontend as / ---
frontend_path = os.path.join(os.path.dirname(__file__), "..", "frontend")
if os.path.isdir(frontend_path):
    app.mount("/", StaticFiles(directory=frontend_path, html=True), name="frontend")

# --- Simple logging & rate limiting middleware ---
@app.middleware("http")
